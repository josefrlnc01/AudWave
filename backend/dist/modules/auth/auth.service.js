...
export class AuthService {
    ...
    static confirmToken = async (token) => {
        try {
    ...
    static authJWTGoogle = async ({ email, name, decodedToken }) => {
        try {
    ...
    static generateTokenForPassword = async (email) => {
        try {
    ...
    static isValidTokenForNewPassword = async (token) => {
        try {
    ...
    static generateNewPassword = async (password, password_confirmation, token) => {
        try {
            const tokenExists = await Token.findOne({
                token
            });
    ...
    static verifyAndSendToken = async (email) => {
        try {
            const user = await User.findOne({ email });
            if (!user) {
                throw new AppError('Credenciales incorrectas', 404);
    ...
    static decodeAndGenerateTokens = async (refreshToken) => {
        try {
            if (!refreshToken) {
