...
export class YoutubeVideoService {
    static insertTranscription = async ({ youtubeVideoText, user, title, duration }) => {
        try {
    ...
    static insertTranslation = async ({ data, user }) => {
        try {
            //Comprobación de documento existente
    ...
    static downloadAudio = async (link) => {
    ...
    static isValidLength = async (id) => {
        ...
        const minutes = getVideoMinutes(videoDuration);
    ...
    static getTranscriptionFromAudio = async (user, ip) => {
        const backendDir = process.cwd();
        const base = path.join(backendDir, 'audio');
        const filepath = base + '.mp3';
        //Obtención del link mediante archivo creado previamente en controller para guardado de links
        const links = JSON.parse(await fs.readFile('link.json', 'utf-8'));
        const videoLink = links.key || null;
        if (!videoLink)
            throw new Error("No se encontró el link del video");
        ...
        await Quota.findOneAndUpdate({ user: user._id, ip }, {
